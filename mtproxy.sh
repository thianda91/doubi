...
Service_mtproxy(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/service/mtproxy_centos" -O /etc/init.d/mtproxy; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/service/mtproxy_debian" -O /etc/init.d/mtproxy; then
			echo -e "${Error} MTProxy服务 管理脚本下载失败 !"
			rm -rf "${file}"
			exit 1
		fi
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/thianda91/doubi/master/mtproxy.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/mtproxy" ]]; then
		rm -rf /etc/init.d/mtproxy
		Service_mtproxy
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/mtproxy.sh" && chmod +x mtproxy.sh
