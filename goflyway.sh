...
Service_goflyway(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/thianda91/doubi/master/service/goflyway_centos -O /etc/init.d/goflyway; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/thianda91/doubi/master/service/goflyway_debian -O /etc/init.d/goflyway; then
			echo -e "${Error} GoFlyway 服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/goflyway
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/thianda91/doubi/master/goflyway.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/goflyway" ]]; then
		rm -rf /etc/init.d/goflyway
		Service_goflyway
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/goflyway.sh" && chmod +x goflyway.sh
