...
Service_dowsdns(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/service/dowsdns_centos" -O /etc/init.d/dowsdns; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/service/dowsdns_debian" -O /etc/init.d/dowsdns; then
			echo -e "${Error} dowsDNS 服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/dowsdns
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/thianda91/doubi/master/dowsdns.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/dowsdns" ]]; then
		rm -rf /etc/init.d/dowsdns
		Service_dowsdns
	fi
		wget -N --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/dowsdns.sh" && chmod +x dowsdns.sh
