...
Service_ct(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/service/cloudt_centos" -O /etc/init.d/cloudt; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/service/cloudt_debian" -O /etc/init.d/cloudt; then
			echo -e "${Error} Cloud Torrent服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/cloudt
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/thianda91/doubi/master/cloudt.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/cloudt" ]]; then
		rm -rf /etc/init.d/cloudt
		Service_ct
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/thianda91/doubi/master/cloudt.sh" && chmod +x cloudt.sh
